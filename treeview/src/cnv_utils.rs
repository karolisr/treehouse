...
pub(super) struct Label {
    text: iced::widget::canvas::Text,
    angle: Option<Float>,
}

#[derive(Debug, Clone, Copy, Default, PartialEq)]
pub(super) struct EdgePoints {
    pt_0: iced::Point,
    pt_1: iced::Point,
}
...
pub(super) fn node_labels(
    nodes: &Vec<NodePoint>, tips: bool, lab_txt_template: &Text,
) -> Vec<Label> {
...
pub(super) fn draw_rectangle(rect: Rectangle, stroke: Stroke, frame: &mut Frame) {
    frame.stroke_rectangle(Point { x: rect.x, y: rect.y }, rect.size(), stroke);
}

pub(super) fn draw_point(point: Point, stroke: Stroke, radius: Float, frame: &mut Frame) {
