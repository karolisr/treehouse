use super::TreeCnv;
use crate::{
    Float, NodePoint, TreeStyle, TreeViewMsg,
...
use iced::{
    ...
    widget::canvas::{
        Action, Geometry, Path, Program, Stroke, Text,
        path::Arc,
...
impl Program<TreeViewMsg> for TreeCnv {
    ...
    fn draw(
        ...
        let g_bounds = self.g_bounds.draw(renderer, bounds.size(), |frame| {
            draw_rectangle(state.clip_rect, frame);
...
//         Some(Action::publish(TreeViewMsg::WindowResized(size.width, size.height)))
//     }
...
//                 );
//             });
//             f.stroke(&path, stroke.with_color(color_danger_base).with_width(SF * 2e0));
//         }
//     });
//     geoms.push(g_bounds);
// }
...
//     f.stroke_rectangle(
//         Point { x: self.clip_rect.x + SF / 2e0, y: self.clip_rect.y + SF / 2e0 },
//         Size { width: self.clip_rect.width, height: self.clip_rect.height - SF - PADDING },
...
//         self.tree_rect.width, self.tree_rect.height, self.center, self.size, self.is_rooted,
//     );
