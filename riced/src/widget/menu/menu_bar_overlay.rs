...
use super::menu_bar_style::MenuStatus;
    ...
    Renderer: renderer::Renderer,
{
    ...
    pub(super) draw_path: &'b MenuDrawPath,
    ...
    Renderer: renderer::Renderer,
{
    fn layout(&mut self, renderer: &Renderer, bounds: Size) -> Node {
        ...
        fn rec<Message, Theme: MenuCatalog, Renderer: renderer::Renderer>(
            renderer: &Renderer, item: &MenuItem<'_, Message, Theme, Renderer>, tree: &mut Tree,
            menu_nodes: &mut Vec<Node>, check_bounds_width: f32, parent_bounds: Rectangle,
            parent_direction: (Direction, Direction), viewport: &Rectangle,
        ) {
            if let Some(menu) = item.menu.as_ref() {
                    ...
                    && active < menu.items.len()
                {
                    ...
                    let next_parent_bounds = {
                        ...
                        let Some(node) =
                            slice_node.children().get(active - menu_state.slice.start_index)
                        else {
                            return;
                        };
                    ...
                    rec(
                        renderer, next_item, next_tree, menu_nodes, check_bounds_width,
                        next_parent_bounds, direction, viewport,
                    );
                }
            }
        }
        ...
        Node::with_children(
            bounds,
            [bar_node, roots_node, Node::with_children(Size::ZERO, menu_nodes)].into(),
        )
    }
    ...
    fn update(
        ...
        fn rec<'a, 'b, Message, Theme: MenuCatalog, Renderer: renderer::Renderer>(
            tree: &mut Tree, item: &mut MenuItem<'a, Message, Theme, Renderer>, event: &Event,
            layout_iter: &mut impl Iterator<Item = Layout<'b>>, cursor: mouse::Cursor,
            renderer: &Renderer, clipboard: &mut dyn Clipboard, shell: &mut Shell<'_, Message>,
            parent_bounds: Rectangle, viewport: &Rectangle, prev_bounds_list: &mut Vec<Rectangle>,
            prev: &mut Index, scroll_speed: ScrollSpeed,
        ) -> RecEvent {
            ...
            let rec_event = if let Some(active) = menu_state.active {
                if active < menu_tree.children.len() {
                    ...
                    let next_parent_bounds = {
                        let Some(layout) =
                            slice_layout.children().nth(active - menu_state.slice.start_index)
                        else {
                    ...
                    rec(
                        next_tree, next_item, event, layout_iter, cursor, renderer, clipboard,
                        shell, next_parent_bounds, viewport, prev_bounds_list,
                        &mut menu_state.active, scroll_speed,
                    )
                } else {
                    RecEvent::Close
                }
            ...
            match rec_event {
                ...
                RecEvent::Close => {
                    if menu_state.pressed || cursor.is_over(prescroll) {
                        menu.update(
                            menu_tree, event, menu_layout, cursor, renderer, clipboard, shell,
                        ...
                        menu.close_event(
                            menu_tree, menu_layout, cursor, parent_bounds, prev_bounds_list, prev,
                        );
                        if prev.is_some() { RecEvent::None } else { RecEvent::Close }
                    }
                }
                RecEvent::None => {
                    if menu_state.pressed || cursor.is_over(prescroll) {
        ...
        let re = rec(
            active_tree, active_root, event, &mut menu_layouts, cursor, renderer, clipboard, shell,
            parent_bounds, &viewport, &mut prev_bounds_list, &mut bar.active_root,
            self.scroll_speed,
        );
    ...
    fn mouse_interaction(
        &self, layout: Layout<'_>, cursor: mouse::Cursor, renderer: &Renderer,
    ) -> mouse::Interaction {
        let bar = self.tree.state.downcast_ref::<MenuBarState>();
        let Some(active) = bar.active_root else {
        ...
        fn rec<'a, 'b, Message, Theme: MenuCatalog, Renderer: renderer::Renderer>(
            tree: &Tree, item: &MenuItem<'a, Message, Theme, Renderer>,
            layout_iter: &mut impl Iterator<Item = Layout<'b>>, cursor: mouse::Cursor,
            renderer: &Renderer, viewport: &Rectangle,
        ) -> mouse::Interaction {
            ...
            menu_state.active.map_or(i, |active| {
                ...
                let next_item = &menu.items[active];
                rec(next_tree, next_item, layout_iter, cursor, renderer, viewport).max(i)
            })
        }

        rec(active_tree, active_root, &mut menu_layouts, cursor, renderer, &viewport)
    }

    fn operate(
        &mut self, layout: Layout<'_>, renderer: &Renderer, operation: &mut dyn Operation<()>,
    ) {
        let bar = self.tree.state.downcast_ref::<MenuBarState>();
        let Some(active) = bar.active_root else {
            return;
        ...
        fn rec<'a, 'b, Message, Theme: MenuCatalog, Renderer: renderer::Renderer>(
            tree: &mut Tree, item: &MenuItem<'a, Message, Theme, Renderer>,
            layout_iter: &mut impl Iterator<Item = Layout<'b>>, renderer: &Renderer,
        ...
        rec(active_tree, active_root, &mut menu_layouts, renderer, operation);
    }

    fn overlay<'c>(
        &'c mut self, layout: Layout<'c>, renderer: &Renderer,
    ...
    fn draw(
        &self, renderer: &mut Renderer, theme: &Theme, style: &renderer::Style, layout: Layout<'_>,
        cursor: mouse::Cursor,
    ) {
        let bar = self.tree.state.downcast_ref::<MenuBarState>();
        let Some(active) = bar.active_root else {
        ...
        fn rec<'a, 'b, Message, Theme: MenuCatalog, Renderer: renderer::Renderer>(
            draw_path: &MenuDrawPath, tree: &Tree, item: &MenuItem<'a, Message, Theme, Renderer>,
            layout_iter: &mut impl Iterator<Item = Layout<'b>>, cursor: mouse::Cursor,
            renderer: &mut Renderer, theme: &Theme, style: &renderer::Style,
            theme_style: &MenuStyle, viewport: &Rectangle,
        ) {
            if let Some(menu) = item.menu.as_ref() {
                let menu_tree = &tree.children[1];
                ...
                menu.draw(
                    draw_path, menu_tree, renderer, theme, style, theme_style, menu_layout, cursor,
                    viewport,
                );
                ...
                if let Some(active) = menu_state.active {
                    ...
                    renderer.with_layer(*viewport, |r| {
                        rec(
                            draw_path, next_tree, next_item, layout_iter, cursor, r, theme, style,
                            theme_style, viewport,
                        );
                    });
                }
            }
        ...
        let theme_style = theme.style(self.class, MenuStatus::Active);

        rec(
            self.draw_path, active_tree, active_root, &mut menu_layouts, cursor, renderer, theme,
            style, &theme_style, &viewport,
        );
    }
