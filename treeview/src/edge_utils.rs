...
pub fn edge_path(nd: &NodeData) -> Path {
    Path::new(|pb| {
        pb.move_to(nd.points.p1);
        pb.line_to(nd.points.p0);
    })
}

pub fn edge_path_cart(nd: &NodeDataCart) -> Path {
    Path::new(|pb| {
        pb.move_to(nd.points.p1);
        pb.line_to(nd.points.p0);
    })
...
pub fn edge_path_pol(nd: &NodeDataPol) -> Path {
    Path::new(|pb| {
        pb.move_to(nd.points.p1);
        pb.line_to(nd.points.p0);
    })
...
pub fn edge_path_vert_cart(nd: &NodeDataCart) -> Path {
    Path::new(|pb| {
...
// impl From<NodeDataCart> for Path {
//     fn from(node_data: NodeDataCart) -> Self {
//         Path::new(|pb| {
...
pub fn edge_path_arc_pol(nd: &NodeDataPol) -> Path {
    Path::new(|pb| {
        pb.move_to(nd.points.p0);
        if nd.angle_parent.is_some() {
            let p_arc = PathArc {
                center: Point::ORIGIN,
                radius: Point::ORIGIN.distance(nd.points.p0),
                start_angle: Radians(nd.angle),
                end_angle: Radians(nd.angle_parent.expect("angle_parent?")),
            };
            pb.arc(p_arc);
        }
    })
...
// impl From<NodeDataPol> for Path {
//     fn from(node_data: NodeDataPol) -> Self {
//         Path::new(|pb| {
//             pb.move_to(node_data.points.p1);
//             pb.line_to(node_data.points.p0);
