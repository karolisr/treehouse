...
pub fn resolve<'a, E, T, Message, Theme, Renderer>(
    axis: Axis, renderer: &Renderer, limits: &Limits, width: Length, height: Length,
    padding: Padding, spacing: Pixels, align_items: Alignment, items: &[E], trees: &mut [T],
) -> Node
where
    ...
    Renderer: renderer::Renderer,
{
    ...
    for (i, (child, tree)) in items.iter().zip(trees).enumerate() {
        ...
        if fill_main_factor != 0 {
            ...
            let min_main = if max_main.is_infinite() { 0.0 } else { max_main };

            let max_cross = if fill_cross_factor == 0 { max_cross } else { cross };
