use super::TreeCnv;
use crate::{
    ChunkEdgeRange, EdgePoints, Float, IndexRange, Label, NodePoint, NodePoints, PADDING,
    treeview::TreeStyle,
};
use dendros::Edge;
use iced::{
...
impl TreeCnv {
    ...
    pub(crate) fn paths_from_chunks(
        ...
        let repr: TreeStyle = self.sel_tree_style_opt;
    ...
    pub(crate) fn visible_tip_idx_range(&self) -> Option<IndexRange> {
        match self.sel_tree_style_opt {
            ...
            TreeStyle::Fan => Some(IndexRange { b: 0, e: self.tree_tip_edges.len() - 1 }),
        }
    }

    pub(crate) fn visible_node_ranges(&self, tip_idx_range: &IndexRange) -> ChunkEdgeRange {
    ...
    pub(crate) fn visible_nodes(
        ...
        let size: Float = match tree_repr {
            TreeStyle::Phylogram => w,
            TreeStyle::Fan => w.min(h) / 2e0,
        };
