use iced::widget::{
    button, checkbox, column as col, container, horizontal_space, row, scrollable, slider, text,
    text_input, toggler, vertical_slider, vertical_space,
};
use iced::{alignment, theme};
use riced::{
    Border, Button, Color, Element, IcedResult, InnerBounds, Length, Menu, MenuDrawPath, MenuItem,
    MenuStatus, MenuStyle, Quad, Radius, Theme, menu_bar, menu_items, primary,
};
...
impl App {
    fn view<'a>(&'a self) -> Element<'a, Message> {
        ...
        let r = row![horizontal_space().width(295), mb, horizontal_space().width(295),]
            .align_y(alignment::Alignment::Center);

        let c = col![vertical_space().height(500), r, vertical_space().height(500),];
        ...
        let back = container(sc).width(Length::Fill).height(Length::Fill).style(back_style);
