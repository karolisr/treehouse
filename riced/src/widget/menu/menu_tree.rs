...
use super::common::*;
use super::flex;
use super::{MenuCatalog, MenuStyle};
use iced::Pixels;
...
{
    pub(super) items: Vec<MenuItem<'a, Message, Theme, Renderer>>,
    ...
    Renderer: renderer::Renderer,
{
    ...
    pub fn new(items: Vec<MenuItem<'a, Message, Theme, Renderer>>) -> Self {
        Self {
            items,
            spacing: Pixels::ZERO,
            max_width: f32::MAX,
    ...
    pub(super) fn tree(&self) -> Tree {
        Tree { tag: self.tag(), state: self.state(), children: self.children() }
    }
}
    ...
    Renderer: renderer::Renderer,
{
    ...
    pub(super) fn tag(&self) -> tree::Tag { tree::Tag::of::<MenuState>() }

    pub(super) fn state(&self) -> tree::State { tree::State::Some(Box::<MenuState>::default()) }

    /// out: \[item_tree...]
    pub(super) fn children(&self) -> Vec<Tree> { self.items.iter().map(MenuItem::tree).collect() }

    /// tree: Tree{menu_state, \[item_tree...]}
    pub(super) fn diff(&self, tree: &mut Tree) {
        tree.diff_children_custom(&self.items, |tree, item| item.diff(tree), MenuItem::tree);
    }
    ...
    pub(super) fn layout(
        &self, tree: &mut Tree, renderer: &Renderer, limits: &Limits, check_bounds_width: f32,
        parent_bounds: Rectangle, parent_direction: (Direction, Direction), viewport: &Rectangle,
        ...
        let aod =
            Aod::new(self.axis, viewport.size(), parent_bounds, parent_direction, self.offset);
        ...
        let slice_node = if slice.start_index == slice.end_index {
            ...
            Node::with_children(
                Size::new(items_node.bounds().width, slice.upper_bound_rel - slice.lower_bound_rel),
                once(start_node)
                    .chain(
    ...
    pub(super) fn update(
        ...
        viewport: &Rectangle, scroll_speed: ScrollSpeed,
    ) -> EventStatus {
        ...
        let status_1 = self.items[slice.start_index..=slice.end_index] // [item...]
            .iter_mut()
    ...
    pub(super) fn draw(
        &self, draw_path: &MenuDrawPath, tree: &Tree, renderer: &mut Renderer, theme: &Theme,
        style: &renderer::Style, theme_style: &MenuStyle, layout: Layout<'_>,
        mut cursor: mouse::Cursor, viewport: &Rectangle,
    ) {
        let mut lc = layout.children();
        let slice_layout = lc.next().unwrap();
        let prescroll = lc.next().unwrap().bounds();
        ...
        if slice.end_index == slice.start_index {
            ...
            if (slice.end_index - slice.start_index) > 1 {
                ...
                let Some(trees) = tree.children.get(slice.start_index + 1..=slice.end_index - 1)
                else {
                    return;
                };

                for ((item, tree), layout) in
                    items.iter().zip(trees.iter()).zip(slice_layout.children().skip(1))
                {
    ...
    pub(super) fn open_event(
        &self, tree: &mut Tree, layout: Layout<'_>, cursor: mouse::Cursor,
    ) -> EventStatus {
        let mut lc = layout.children();
        let slice_layout = lc.next().unwrap();
        // let prescroll = lc.next().unwrap().bounds();
    ...
    pub(super) fn close_event(
        &self, tree: &mut Tree, layout: Layout<'_>, cursor: mouse::Cursor,
        parent_bounds: Rectangle, prev_bounds_list: &[Rectangle], prev: &mut Index,
    ) {
        let mut lc = layout.children();
...
/// Item inside a [`Menu`]
#[must_use]
pub struct MenuItem<'a, Message, Theme, Renderer>
where
    Theme: MenuCatalog,
    Renderer: renderer::Renderer,
{
    ...
    Renderer: renderer::Renderer,
{
    ...
    pub fn with_menu(
        ...
        Self { item: item.into(), menu: Some(Box::new(menu)) }
    }
...
impl<Message, Theme, Renderer> MenuItem<'_, Message, Theme, Renderer>
where
    Theme: MenuCatalog,
    Renderer: renderer::Renderer,
{
    pub(super) fn tag(&self) -> tree::Tag { tree::Tag::stateless() }
    pub(super) fn state(&self) -> tree::State { tree::State::None }
    ...
    pub(super) fn operate(
        ...
        self.item.as_widget().operate(&mut tree.children[0], layout, renderer, operation);
    }
    ...
    pub(super) fn overlay<'b>(
        ...
        self.item.as_widget_mut().overlay(
            ...
            layout,
            renderer,
            &Rectangle { x: 0e0, y: 0e0, width: 1e2, height: 1e2 },
            translation,
        )
    }
}
...
impl Aod {
    ...
    fn adaptive(
        parent_pos: f32, parent_size: f32, child_size: f32, max_size: f32, offset: f32,
        overlap: bool, direction: Direction,
    ) -> (f32, f32, Direction) {
        /*
    ...
    /// Returns (child position, offset position, child direction)
    fn resolve(
        &self, parent_bounds: Rectangle, children_size: Size, viewport_size: Size,
    ) -> (Point, Point, (Direction, Direction)) {
        let (x, ox, dx) = Self::adaptive(
            parent_bounds.x, parent_bounds.width, children_size.width, viewport_size.width,
            self.horizontal_offset, self.horizontal_overlap, self.horizontal_direction,
        );
        let (y, oy, dy) = Self::adaptive(
            parent_bounds.y, parent_bounds.height, children_size.height, viewport_size.height,
            self.vertical_offset, self.vertical_overlap, self.vertical_direction,
        );
    ...
    fn new(
        axis: Axis, viewport: Size, parent_bounds: Rectangle,
        parent_direction: (Direction, Direction), offset: f32,
    ) -> Self {
        ...
        match axis {
            Axis::Horizontal => {
                ...
                let vertical_direction =
                    if pvcenter < vcenter { Direction::Positive } else { Direction::Negative };
                Self {
            ...
            Axis::Vertical => {
                let horizontal_direction =
                    if phcenter < hcenter { Direction::Positive } else { Direction::Negative };
...
fn process_scroll_event(
    menu_state: &mut MenuState, prescroll_children_bounds: Rectangle, delta: mouse::ScrollDelta,
    scroll_speed: ScrollSpeed, viewport_size: Size,
) {
...
impl MenuSlice {
    fn new(items_node: &Node, translation: Vector, viewport: Size, scroll_offset: f32) -> Self {
        ...
        Self { start_index, end_index, lower_bound_rel, upper_bound_rel }
    }
}
...
fn clip_node_y(node: &Node, height: f32, offset: f32) -> Node {
    ...
    Node::with_children(
        ...
        node.children().iter().map(|n| n.clone().translate([0.0, -offset])).collect(),
    )
