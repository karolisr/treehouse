...
use super::menu_bar_style::{MenuStatus, StyleFn};
...
{
    roots: Vec<MenuItem<'a, Message, Theme, Renderer>>,
    spacing: Pixels,
    padding: Padding,
    width: Length,
    height: Length,
    check_bounds_width: f32,
    draw_path: MenuDrawPath,
    ...
    Renderer: renderer::Renderer,
{
    ...
    pub fn new(mut roots: Vec<MenuItem<'a, Message, Theme, Renderer>>) -> Self {
        for i in &mut roots {
        ...
        Self {
            ...
            draw_path: MenuDrawPath::FakeHovering,
            scroll_speed: ScrollSpeed { line: 60.0, pixel: 1.0 },
            class: Theme::default(),
        }
    }
    ...
    pub fn draw_path(mut self, draw_path: MenuDrawPath) -> Self {
    ...
    pub fn style(mut self, style: impl Fn(&Theme, MenuStatus) -> MenuStyle + 'a) -> Self
    where
        Theme::Class<'a>: From<StyleFn<'a, Theme, MenuStyle>>,
    {
        self.class = (Box::new(style) as StyleFn<'a, Theme, MenuStyle>).into();
        self
    }
    ...
    Renderer: renderer::Renderer,
{
    ...
    fn tag(&self) -> tree::Tag { tree::Tag::of::<MenuBarState>() }

    fn state(&self) -> tree::State { tree::State::Some(Box::<MenuBarState>::default()) }

    /// \[Tree{stateless, \[widget_state, menu_state]}...]
    fn children(&self) -> Vec<Tree> { self.roots.iter().map(MenuItem::tree).collect::<Vec<_>>() }

    /// tree: Tree{bar_state, \[item_tree...]}
    fn diff(&self, tree: &mut Tree) {
        tree.diff_children_custom(&self.roots, |tree, item| item.diff(tree), MenuItem::tree);
    }
    ...
    fn draw(
        ...
        layout: Layout<'_>, mut cursor: mouse::Cursor, viewport: &Rectangle,
    ) {
        let styling = theme.style(&self.class, MenuStatus::Active);
        renderer.fill_quad(
            renderer::Quad {
        ...
        if state.open
            && let Some(active) = state.active_root
        {
    ...
    fn overlay<'b>(
        &'b mut self, tree: &'b mut Tree, layout: Layout<'_>, _renderer: &Renderer,
        _viewport: &Rectangle, translation: iced::Vector,
...
{
    fn from(value: MenuBar<'a, Message, Theme, Renderer>) -> Self { Self::new(value) }
}
